...
public class ProdBomPlmController extends BaseController {
     ...
     */
    @RequestMapping(value = "findMattersAndMidProd", method = RequestMethod.POST)
    @ResponseBody
    public String findMattersAndMidProd(String pNo, String mpno) {
        BaseResult<ProdBomPlmDto> baseResult = new BaseResult<>();
        try {
            //            ProdBomPlmDto prodBomPlmDto = prodBomPlmService.findMattersAndMidProd(pNo, mpno);
    ...
    @ResponseBody
    public String SearchBeforeAddMatters(String codeOrName) {
        BaseResult<List<MattersStoreDto>> baseResult = new BaseResult<>();
        try {
            List<MattersStoreDto> list = prodBomPlmService.SearchBeforeAddMatters(codeOrName);
            baseResult.setResult(true);
            baseResult.setModel(list);
        } catch (BusinessException be) {
            LOGGER.debug("业务异常" + be);
